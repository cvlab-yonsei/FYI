...
class BatchDiffAug():
    def __init__(self):
        self.aug_mode = 'S' #'multiple or single'
        self.prob_flip = 1.0
        self.ratio_scale = 1.2 / 2
        self.ratio_rotate = 15.0 / 2
        self.brightness = 1.0 / 2
        self.saturation = 2.0 / 2
        self.contrast = 0.5 / 2
        self.batchmode = False
        self.latestseed = -1
...
def fixed_scale(x, ratio):
    # x>1, max scale
    # sx, sy: (0, +oo), 1: orignial size, 0.5: enlarge 2 times
    sx = x.shape[0]/2 * (ratio - 1.0/ratio) + 1.0/ratio
    sy = x.shape[0]/2 * (ratio - 1.0/ratio) + 1.0/ratio
    theta = [[[sx, 0,  0],
            [0,  sy, 0],] for i in range(x.shape[0])]
