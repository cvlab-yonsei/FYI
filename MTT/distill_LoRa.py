...
def main(args):
    ...
    args.ba_param = BatchDiffAug()

    if args.batch_aug in ['FlipBatch']:
        args.batch_train = args.batch_train//2

    dsa_params = args.dsa_param
    ba_params = args.ba_param
    if args.zca:
    ...
    ''' training '''
    image_init = image_syn.detach().to(args.device).requires_grad_(False)
    shape_syn = image_init.shape
    lora_A = torch.rand((shape_syn[0], shape_syn[1] * shape_syn[2], 10)).to(args.device).requires_grad_(True)
    lora_B = torch.zeros((shape_syn[0], 10, shape_syn[3])).to(args.device).requires_grad_(True)


    syn_lr = syn_lr.detach().to(args.device).requires_grad_(True)
    optimizer_img = torch.optim.SGD([lora_A, lora_B], lr=args.lr_img, momentum=0.5)
    ...
    for it in range(0, args.Iteration+1):
        ...
        image_syn = image_init + torch.bmm(lora_A, lora_B).reshape(shape_syn)
        ...
        lora_A_grad = torch.zeros(lora_A.shape).to(args.device).requires_grad_(False)
        lora_B_grad = torch.zeros(lora_B.shape).to(args.device).requires_grad_(False)
        ...
        for il in range(args.syn_steps):
            ...
            x_init = image_init[these_indices]
            x_A = lora_A[these_indices]
            x_B = lora_B[these_indices]
            x = torch.bmm(x_A, x_B).reshape(x_init.shape) + x_init
            this_y = y_hat[these_indices]

            original_x_A_list.append(x_A)
            original_x_B_list.append(x_B)
        ...
        for il in range(args.syn_steps):
            ...
            gradients = torch.autograd.grad(per_batch_loss, [original_x_A_list[il], original_x_B_list[il]], retain_graph=False)

            with torch.no_grad():
                lora_A_grad[indices_chunks_copy[il]] += gradients[0]
                lora_B_grad[indices_chunks_copy[il]] += gradients[1]
        ...
        lora_A.grad = lora_A_grad.detach().clone()
        lora_B.grad = lora_B_grad.detach().clone()

        del lora_A_grad, lora_B_grad
        del lr_grad
...
if __name__ == '__main__':
    ...
    parser.add_argument('--LoRadim', type=int, default=10, help='Low Rank Approximation Dimension')
