...
def main(args):
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(teacher_optim, T_max=args.train_epochs, eta_min=0.0001)

    for e in range(args.train_epochs):

        train_loss, train_acc = epoch("train", dataloader=trainloader, net=teacher_net, optimizer=teacher_optim,
                                    criterion=criterion, args=args, aug=args.aug)
    ...
    # save teacher model
    torch.save(teacher_net.state_dict(), os.path.join(save_dir, f'{args.model}.pth'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--aug', type=bool, default=True, help='whether to use augmentation')
