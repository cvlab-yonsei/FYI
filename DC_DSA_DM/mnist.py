...
def main():
    ...
    torch.cuda.manual_seed_all(args.seed)  # 추가
    ...
    # The list of iterations when we visualize the gradient heatmap.
    grad_heatmap_pool = [i for i in range(0, args.Iteration+1, 10)]
    ...
    for exp in range(args.num_exp):
        ...
        # Augment list of real data with their flipped versions
        images_all = images_all + [torch.flip(torch.unsqueeze(dst_train[i][0], dim=0), dims=[3]) for i in range(len(dst_train))]
        labels_all = labels_all + [dst_train[i][1] for i in range(len(dst_train))]
        ...
        for it in range(args.Iteration+1):
            ...
            if it in eval_it_pool:
                for model_eval in model_eval_pool:
                    ...
                    wandb.log({'Accuracy_default/{}'.format(model_eval): np.mean(accs)}, step=it)
                    wandb.log({'Std_default/{}'.format(model_eval): np.std(accs)}, step=it)
                    ...
                    wandb.log({'Accuracy_flip/{}'.format(model_eval): np.mean(accs_Flip)}, step=it)
                    wandb.log({'Std_flip/{}'.format(model_eval): np.std(accs_Flip)}, step=it)
                    ...
                    wandb.log({'Accuracy_flipBatch/{}'.format(model_eval): np.mean(accs_FlipBatch)}, step=it)
                    wandb.log({'Std_flipBatch/{}'.format(model_eval): np.std(accs_FlipBatch)}, step=it)
            ...
            if it in grad_heatmap_pool:
                ...
                wandb.log({"Synthetic_Images": wandb.Image(make_grid(image_syn_vis, nrow=10))}, step=it)
            ...
            for ol in range(args.outer_loop):
                ...
                if it in grad_heatmap_pool:
                    # Calculate gradient of loss w.r.t. img_syn
                    grad = torch.autograd.grad(loss, image_syn, create_graph=True)[0].detach().cpu() # N, C, H, W = (10, 1, 28, 28)
                    # min-max normalize gradient along the first dimension (N)
                    grad_norm = (grad - grad.min(dim=0, keepdim=True)[0]) / (grad.max(dim=0, keepdim=True)[0] - grad.min(dim=0, keepdim=True)[0])
                    # Save gradient heatmap
                    save_name = os.path.join(args.save_path, 'gradnorm_%s_%s_%s_%dipc_exp%d_iter%d.png'%(args.method, args.dataset, args.model, args.ipc, exp, it))
                    save_image(grad_norm, save_name, nrow=10)
                    wandb.log({"Gradient heatmap": wandb.Image(make_grid(grad_norm, nrow=10))}, step=it)
